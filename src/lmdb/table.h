...
namespace lmdb
{
    ...
    struct table
    {
        ...
        //! Open `name` with `flags`, `key_cmp` and `value_cmp` using `write_txn`.
        expect<MDB_dbi> open(MDB_txn& write_txn) const noexcept;
    };

    //! Herlp for grouping typical LMDB DBI options when key and value are fixed types.
    ...
    struct basic_table : table
    {
        ...
        /*!
            ...
            \tparam F the field type within `U` that is being extracted.
            \tparam offset The offset to `F` within `U`.

            \return Value of type `F` at `offset` within `value` of type `U`.
        */
        ...
        /*!
            \param[in] txn Linked to the cursor used by the return value.
            \param tbl Linked to a new cursor, when `cur == nullptr`.
            \param cur Optionally reuse an exisiting cursor. Returned in object
                on success, otherwise destroyed.
            \return A handle to the first key/value within `tbl`.
        */
        template<typename D>
        expect<key_stream<K, V, D>>
        static get_key_stream(std::unique_ptr<MDB_cursor, D> cur = nullptr) noexcept
        {
            if (cur)
            {
                MDB_val key;
        ...
        static get_value_stream(K const& key, std::unique_ptr<MDB_cursor, D> cur = nullptr) noexcept
        {
            if (cur)
            {
